...
def save_link(df, url, title, description, tags, priority, mode):
...
def delete_selected_links(df, selected_ids, excel_file, mode):
    ...
    try:
        updated_df = df[~df['id'].isin(selected_ids)].reset_index(drop=True)
...
def process_bookmark_file(df, uploaded_file, mode):
    ...
    try:
        ...
        if file_type in ['xlsx', 'csv']:
            ...
            # Assume columns: URL, Title, Description, Tags, Folder, etc.
        ...
        texts = [f"{link['title']} {link['description']} {' '.join(link['tags'])}" for link in links]
        vectorizer = TfidfVectorizer(max_features=1000, stop_words='english')
        X = vectorizer.fit_transform(texts)
        kmeans = KMeans(n_clusters=min(5, len(links)), random_state=42)
        ...
        for i, link in enumerate(links):
            ...
            # Fetch metadata for better tags
            ...
            link['tags'] = list(set(link['tags']))  # Remove duplicates
