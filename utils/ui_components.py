...
def add_link_section(df, excel_file, mode):
    ...
    # Create tabs for single link and file upload
    single_tab, upload_tab = st.tabs(["Add Single Link", "Upload Bookmarks"])
    
    # Single Link Tab
    with single_tab:
        with st.form(key="add_single_link_form"):
            st.markdown("<h4>Enter Link Details</h4>", unsafe_allow_html=True)
