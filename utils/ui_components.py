...
def add_link_section(df, excel_file, mode):
    ...
    with st.form(key="add_link_form"):
        st.markdown("<h4>Add Link</h4>", unsafe_allow_html=True)
        
        # URL input and metadata fetch
        ...
        if fetch_button and url:
            try:
                ...
                st.rerun()  # Refresh to update form fields
            ...
            if url:
                ...
                if mode in ["admin", "guest"] and excel_file:
                    if save_data(new_df, excel_file):
                        ...
                        st.balloons()
                        st.session_state['fetched_metadata'] = {}  # Clear metadata
                    else:
                    ...
                    if mode in ["admin", "guest"] and excel_file:
                        if save_data(new_df, excel_file):
                            ...
                            if new_df['is_duplicate'].any():
                                st.warning("⚠️ Some uploaded URLs are duplicates. Review in the exported Excel file.")
                            st.balloons()
                        else:
...
def browse_section(df, excel_file, mode):
    ...
    if not filtered_df.empty:
        ...
        if st.button("Delete Selected Links", key="delete_button"):
            if selected_numbers:
                # Map numbers back to link_ids for deletion
