...
def add_link_section(df, excel_file, mode):
    """Section for adding new links or uploading bookmark files with domain-based tag pre-filling"""
    apply_css()
    ...
    with tab1:
            ...
            if domain:
                ...
                if not matching_rows.empty:
                    ...
                    if prefilled_tags:
                        ...
                        logging.debug(f"Pre-filled tags {prefilled_tags} for domain {domain}")
                else:
        ...
        with st.form("single_url_form", clear_on_submit=True):
            ...
            if submitted:
                logging.debug(f"Form submitted: URL={url}, Title={title}, Tags={tags}, Mode={mode}, Excel={excel_file}")
                if not url:
                    ...
                    if new_df is not None:
                        if mode in ["admin", "guest"] and excel_file:
                            if save_data(new_df, excel_file):
                                ...
                                st.error(f"❌ Failed to save link to {excel_file}. Check Google Drive configuration or download the file.")
                    ...
                    try:
                        ...
                        if mode in ["admin", "guest"] and excel_file:
                            if save_data(new_df, excel_file):
                                ...
                                st.error(f"❌ Failed to save bookmarks to {excel_file}. Check Google Drive configuration or download the file.")
